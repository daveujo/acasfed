/*
 * A.C.A.S (Advanced Chess Assistance System) - Browser Extension Content Script
 * Version: 2.3.7
 * 
 * This is the browser extension version that bypasses CSP restrictions by using
 * chrome.runtime messaging to communicate with a background service worker that
 * handles WebSocket connections.
 * 
 * Original userscript: https://greasyfork.org/en/scripts/471519
 * Homepage: https://federico98x.github.io/CHESS/
 * License: GPL-3.0
 */

// Extension-specific GM_* API replacements
// These use chrome.runtime.sendMessage to communicate with the background service worker
async function GM_getValue(key, defaultValue) {
  return new Promise((resolve) => {
    chrome.runtime.sendMessage(
      { type: 'gm-getValue', key, defaultValue },
      (response) => {
...
function GM_setClipboard(text) {
  ...
  navigator.clipboard.writeText(text).catch(() => {
    console.warn('[ACAS] Failed to copy to clipboard');
  });
}

function GM_registerMenuCommand(text, callback, accessKey) {
  // Extension context menus would require additional setup
  // For now, we'll log that this feature is not available
  console.log('[ACAS] Menu command registered (extension):', text);
}

// Polyfill for GM.* async API (same as GM_* but wrapped in Promise if needed)
const GM = {
...
// No unsafeWindow in extensions, use window directly
...
// Extension version: WebSocket is handled by background service worker
// No need for TrueNativeWebSocket bypass since service workers aren't subject to page CSP
const TrueNativeWebSocket = WebSocket; // Not used in extension, but kept for compatibility
...
(async () => {
// Extension version: No need for LOAD_LEGACY_GM_SUPPORT, GM_* functions are already defined above
/*
┏┓┓ ┏┓┳┓┏┓┓
┃┓┃ ┃┃┣┫┣┫┃
┗┛┗┛┗┛┻┛┛┗┗┛
...
// External Engine State (managed by background service worker)
let externalEngineReady = false;
let externalEngineConnected = false;
const externalEngineListeners = new Set();

// Subscribe to WebSocket messages from background
chrome.runtime.sendMessage({ type: 'ws-subscribe' });

// Listen for WebSocket messages from background service worker
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === 'ws-connected') {
        externalEngineConnected = message.connected;
        updateExternalEngineState();
        if(debugModeActivated) console.log('[ExtEngine] Connected:', message.connected);
    } else if (message.type === 'ws-disconnected') {
        ...
        externalEngineReady = message.ready;
        updateExternalEngineState();
        if(debugModeActivated) console.log('[ExtEngine] Ready:', message.ready);
    } else if (message.type === 'ws-message') {
        // Route to listeners
        for (const fn of externalEngineListeners) {
            try { fn({ data: message.data }); } catch(x) {
                if(debugModeActivated) console.error('[ExtEngine] Listener error:', x);
            }
        }
    } else if (message.type === 'ws-error') {
        if(debugModeActivated) console.error('[ExtEngine] Error:', message.error);
    }
});
...
function connectExternalEngine() {
    ...
    if(debugModeActivated) console.log('[ExtEngine] Connecting via background service worker to', engineUrl);

    // Send connection request to background service worker
    chrome.runtime.sendMessage({
        type: 'ws-connect',
        url: engineUrl
    }, (response) => {
        if (response && response.success) {
            if(debugModeActivated) console.log('[ExtEngine] Connection request sent');
        } else {
            if(debugModeActivated) console.error('[ExtEngine] Failed to send connection request');
        }
    });
}

function disconnectExternalEngine() {
    // Send disconnection request to background service worker
    chrome.runtime.sendMessage({ type: 'ws-disconnect' }, (response) => {
        if (response && response.success) {
            externalEngineConnected = false;
            externalEngineReady = false;
            updateExternalEngineState();
...
function sendToEngine(cmd) {
    ...
    if (useExtEngine && externalEngineConnected) {
        if(debugModeActivated) console.log('[ExtEngine] Sending via background:', cmd);
        chrome.runtime.sendMessage({
            type: 'ws-send',
            command: cmd
        }, (response) => {
            if (!response || !response.success) {
                if(debugModeActivated) console.error('[ExtEngine] Failed to send command');
            }
        });
        return true;
    }
